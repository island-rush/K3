...
There are many methods of running / hosting a MySQL server. Once the database exists, please create a user/password for the game to use. Set these values in the env variables, or for easier/quicker setup...manually set them in the ./src/server/database.ts file. Some defaults already exist here, simply overwrite them or add your own according to the examples.
...
Note: Course Director default credentials are set within ./src/server/adminFunctions/adminLoginVerfy.ts
...
Please note that production environments will build the client, creating static files in ./src/react-client/build for the server to use. If the NODE_ENV is set to 'production', the backend will use this. Otherwise, it will redirect /game to localhost:3000, as this is the default port for the react server. This can be changed in the ./src/server/router.ts file. It is possible to manually build the client using the following command.
