...
//prettier-ignore
import { AIRCRAFT_CLICK, ANTISAT_HIT_ACTION, ANTISAT_SELECTED, ATC_SCRAMBLE_SELECTED, ATC_SCRAMBLE_SELECTING, BATTLE_FIGHT_RESULTS, BATTLE_PIECE_SELECT, BIO_WEAPON_SELECTED, BIO_WEAPON_SELECTING, BOMBARDMENT_SELECTED, BOMBARDMENT_SELECTING, CANCEL_PLAN, COMBAT_PHASE, COMM_INTERRUPT_SELECTING, COMM_INTERRUP_SELECTED, CYBER_DEFENSE_CHECK, CYBER_DEFENSE_SELECTED, DELETE_PLAN, DRONE_SWARM_SELECTED, DRONE_SWARM_SELECTING, ENEMY_PIECE_SELECT, EVENT_BATTLE, EVENT_REFUEL, GOLDEN_EYE_SELECTED, GOLDEN_EYE_SELECTING, INITIAL_GAMESTATE, INNER_PIECE_CLICK_ACTION, INNER_TRANSPORT_PIECE_CLICK_ACTION, INSURGENCY_SELECTED, INSURGENCY_SELECTING, MAIN_BUTTON_CLICK, MISSILE_DISRUPT_SELECTED, MISSILE_DISRUPT_SELECTING, MISSILE_SELECTED, MISSILE_SELECTING, NEWS_PHASE, NEW_ROUND, NO_MORE_BATTLES, NUKE_SELECTED, NUKE_SELECTING, OUTER_PIECE_CLICK_ACTION, PIECE_CLOSE_ACTION, PIECE_OPEN_ACTION, PLACE_PHASE, PLAN_WAS_CONFIRMED, PURCHASE_PHASE, RAISE_MORALE_SELECTED, RAISE_MORALE_SELECTING, REMOTE_SENSING_HIT_ACTION, REMOTE_SENSING_SELECTED, REMOTE_SENSING_SELECTING, RODS_FROM_GOD_SELECTED, RODS_FROM_GOD_SELECTING, SEA_MINE_SELECTED, SEA_MINE_SELECTING, SET_USERFEEDBACK, SHOP_PURCHASE, SHOP_REFUND, SHOP_TRANSFER, SLICE_CHANGE, START_PLAN, TARGET_PIECE_SELECT, UPDATE_FLAGS, BLUE_TEAM_ID, RED_TEAM_ID } from "../../../../constants";
import { CyberDefenseCheckAction, UserfeedbackAction, UserfeedbackState, UpdateFlagAction } from '../../../../types';
...
export function userFeedbackReducer(state = initialUserFeedback, action: AnyAction) {
    ...
    switch (type) {
        ...
        case UPDATE_FLAGS:
            // TODO: make a better victory condition (don't use the word Blue team, use zuun or something)
            const { flag0, flag1, flag11, flag12 } = (action as UpdateFlagAction).payload;
            if (flag0 === BLUE_TEAM_ID && flag1 === BLUE_TEAM_ID && flag11 === RED_TEAM_ID && flag12 === RED_TEAM_ID) {
                return 'Tie! (almost forgot to code this in, probably wont happen anyways)';
            } else if (flag0 === BLUE_TEAM_ID && flag1 === BLUE_TEAM_ID) {
                return 'Blue Team Victory!';
            } else if (flag11 === RED_TEAM_ID && flag12 === RED_TEAM_ID) {
