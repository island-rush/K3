...
//prettier-ignore
import {POSITION_SELECT,PIECE_CLICK,PIECE_CLEAR_SELECTION,START_PLAN,CANCEL_PLAN,PLANNING_SELECT,PLAN_WAS_CONFIRMED,DELETE_PLAN,UNDO_MOVE,CONTAINER_MOVE,INITIAL_GAMESTATE,SLICE_CHANGE,PURCHASE_PHASE,NEWS_PHASE,BATTLE_PIECE_SELECT,ENEMY_PIECE_SELECT,BATTLEPOPUP_MINIMIZE_TOGGLE,TARGET_PIECE_SELECT,EVENT_BATTLE,BATTLE_FIGHT_RESULTS,NO_MORE_EVENTS,EVENT_REFUEL,CLEAR_BATTLE,MENU_SELECT,HIGHLIGHT_POSITIONS,TANKER_CLICK,AIRCRAFT_CLICK,UNDO_FUEL_SELECTION,REFUEL_RESULTS,NEWSPOPUP_MINIMIZE_TOGGLE,REFUELPOPUP_MINIMIZE_TOGGLE,RODS_FROM_GOD_SELECTING,RODS_FROM_GOD_SELECTED,NEW_ROUND,REMOTE_SENSING_SELECTING,REMOTE_SENSING_SELECTED,PLACE_PHASE,INSURGENCY_SELECTED,INSURGENCY_SELECTING,BIO_WEAPON_SELECTED,BIO_WEAPON_SELECTING,RAISE_MORALE_SELECTED,RAISE_MORALE_SELECTING,COMM_INTERRUP_SELECTED,COMM_INTERRUPT_SELECTING,GOLDEN_EYE_SELECTING,GOLDEN_EYE_SELECTED,PIECE_OPEN_ACTION,PIECE_CLOSE_ACTION,OUTER_PIECE_CLICK_ACTION,INNER_PIECE_CLICK_ACTION,INNER_TRANSPORT_PIECE_CLICK_ACTION} from "../actions/actionTypes";
...
function gameboardMetaReducer(state = initialGameboardMeta, { type, payload }: { type: string; payload: any }) {
    ...
    switch (type) {
        ...
        case BATTLE_PIECE_SELECT:
            ...
            stateDeepCopy.battle.selectedBattlePiece =
                payload.battlePiece.piece.pieceId === lastSelectedBattlePiece ? -1 : payload.battlePiece.piece.pieceId;
            stateDeepCopy.battle.selectedBattlePieceIndex =
                payload.battlePiece.piece.pieceId === lastSelectedBattlePiece ? -1 : payload.battlePieceIndex;
            break;
        ...
        case BATTLE_FIGHT_RESULTS:
            ...
            for (let x = 0; x < stateDeepCopy.battle.friendlyPieces.length; x++) {
                ...
                let currentRecord = payload.masterRecord.find((record: any, index: any) => {
            ...
            for (let z = 0; z < stateDeepCopy.battle.enemyPieces.length; z++) {
                ...
                let currentRecord = payload.masterRecord.find((record: any, index: number) => {
