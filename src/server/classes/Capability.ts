...
export class Capability {
    ...
    static async checkSeaMineHit(gameId: number): Promise<number[]> {
        const queryString =
        ...
        if (results.length !== 0) {
            ...
            const arrayOfPos = await Capability.checkSeaMineHit(gameId); // TODO: probably a better way instead of recursive, but makes sense here since we keep calling it until results.length == 0
